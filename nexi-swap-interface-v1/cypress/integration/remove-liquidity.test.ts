describe('Remove Liquidity', () => {
  ...
  it('cryto remove', () => {
    cy.visit('/remove/NEXI/0x0e09fabb73bd3ade0a17ecc321fd13a19e81ce82')
    cy.get('#remove-liquidity-tokena-symbol').should('contain.text', 'NEXI')
    cy.get('#remove-liquidity-tokenb-symbol').should('contain.text', 'CAKE')
  })

  it('bnb remove swap order', () => {
    cy.visit('/remove/0x0e09fabb73bd3ade0a17ecc321fd13a19e81ce82/NEXI')
    cy.get('#remove-liquidity-tokena-symbol').should('contain.text', 'CAKE')
    cy.get('#remove-liquidity-tokenb-symbol').should('contain.text', 'NEXI')
  })
